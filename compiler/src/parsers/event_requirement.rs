...
impl EventRequirement {
        ...
        match config.resources.get_key_value(resource) {
          Some((_, kind)) => {
            let result = match kind {
              Integer => { Self::parse_amount_and_condition::<&str, i32>(params[2], Condition::HigherThan) },
              Float => { Self::parse_amount_and_condition::<&str, f32>(params[2], Condition::HigherThan) }
  ...
  fn parse_amount_and_condition<T, U>(data: T, default_condition: Condition) -> Result<(String, Condition), String> 
  where 
    T: AsRef<str>,
    U: FromStr + Display
  {
...
mod test {
  ...
  fn parses_integer_resource() {
    ...
    assert_eq!(result, cuentitos_common::EventRequirement::Resource {
  ...
  fn parses_items() {
    ...
    assert_eq!(result, cuentitos_common::EventRequirement::Item {
  ...
  fn parses_reputations() {
    ...
    assert_eq!(result, cuentitos_common::EventRequirement::Reputation {
  ...
  fn parses_time_of_day() {
    ...
    assert_eq!(result, cuentitos_common::EventRequirement::TimeOfDay{ id: cuentitos_common::TimeOfDay::Morning, condition: Equals });

    let result = EventRequirement::parse("time_of_day noon", &config).unwrap();
    assert_eq!(result, cuentitos_common::EventRequirement::TimeOfDay{ id: cuentitos_common::TimeOfDay::Noon, condition: Equals });

    let result = EventRequirement::parse("time_of_day evening", &config).unwrap();
    assert_eq!(result, cuentitos_common::EventRequirement::TimeOfDay{ id: cuentitos_common::TimeOfDay::Evening, condition: Equals });

    let result = EventRequirement::parse("time_of_day night", &config).unwrap();
    assert_eq!(result, cuentitos_common::EventRequirement::TimeOfDay{ id: cuentitos_common::TimeOfDay::Night, condition: Equals });
  }
  ...
  fn error_on_wrong_time_of_day() {
    ...
    assert_eq!(Err("'tonight' is not a valid time of day".to_string()), result);
  }
  ...
  fn parses_event() {
    ...
    assert_eq!(result, cuentitos_common::EventRequirement::Event { id: "an_event".to_string(), condition: Depends });

    let result = EventRequirement::parse("event !an_event", &config).unwrap();
    assert_eq!(result, cuentitos_common::EventRequirement::Event { id: "an_event".to_string(), condition: MutEx });
  }
  ...
  fn parses_decision() {
    ...
    assert_eq!(result, cuentitos_common::EventRequirement::Decision { id: "a_decision".to_string(), condition: Depends });

    let result = EventRequirement::parse("decision !a_decision", &config).unwrap();
    assert_eq!(result, cuentitos_common::EventRequirement::Decision { id: "a_decision".to_string(), condition: MutEx });
  }
  ...
  fn parses_tile() {
    ...
    assert_eq!(result, cuentitos_common::EventRequirement::Tile { id: "forest".to_string(), condition: Depends });

    let result = EventRequirement::parse("tile !forest", &config).unwrap();
    assert_eq!(result, cuentitos_common::EventRequirement::Tile { id: "forest".to_string(), condition: MutEx });
  }
