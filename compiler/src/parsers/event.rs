          ...
          Ok(modifier) => { 
            if in_choice && in_result {
              current_result.modifiers.push(modifier);
                ...
                return Err(format!("[{}] found modifier outside a result: {}", line_number, line))
              }
          },
          Err(error) => return Err(format!("[{}] {}", line_number, error))
        }
...
mod test {
  ...
  fn parse_parses_choices()
  {
    ...
    let event = Event::parse(event, &Config::default()).unwrap();
    println!("{:?}", event);
    assert_eq!(event.choices[0], EventChoice {
      text: "An Option without requirements".to_string(),
      ..Default::default()
    });

    assert_eq!(event.choices[1], EventChoice {
  ...
  fn parse_parses_requirements()
  {
    ...
    assert_eq!(event.requirements, vec![
      ...
      EventRequirement::Item { id: "wooden_figure".to_string(), condition: Condition::Equals, amount: 1.to_string() },
      EventRequirement::Reputation { id: "rep-1".to_string(), condition: Condition::HigherThan, amount: 5.to_string() },
      EventRequirement::TimeOfDay { id: TimeOfDay::Night, condition: Condition::Equals },
      EventRequirement::Event { id: "choices".to_string(), condition: Condition::MutEx },
      EventRequirement::Decision { id: "a_decision".to_string(), condition: Condition::Depends },
      EventRequirement::Tile { id: "forest".to_string(), condition: Condition::Depends }
    ]);
  }
  ...
  fn parse_parses_modifiers()
  {
    ...
    assert_eq!(event.choices[0].results[0].modifiers, vec![
      Modifier::Item { id: "wooden_figure".to_string(), amount: 1.to_string() },
      Modifier::Item { id: "wooden_figure".to_string(), amount: (-1).to_string() },
      Modifier::Resource {
        resource: Resource { id: "health".to_string(), kind: Integer },
        amount: (-2).to_string()
      },
      Modifier::Resource {
        resource: Resource { id: "health".to_string(), kind: Integer },
      ...
      Modifier::Resource {
        resource: Resource { id: "happy".to_string(), kind: Bool },
      ...
      Modifier::Reputation { id: "rep-1".to_string(), amount: 2.to_string() },
      Modifier::Reputation { id: "rep-2".to_string(), amount: (-2).to_string() },
