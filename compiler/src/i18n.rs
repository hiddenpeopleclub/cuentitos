...
use cuentitos_common::{LanguageDb, Database};
...
impl I18n {
  pub fn process<T, U>(database: &mut Database, base_path: T, destination_path: U) -> std::io::Result<cuentitos_common::I18n> 
  where 
    T: AsRef<Path>,
    U: AsRef<Path>,
  {
    let mut i18n = cuentitos_common::I18n::default();
    i18n.default_locale = database.config.default_locale.clone();
    i18n.locales = database.config.locales.clone();
    ...
    for block in &mut database.blocks {
