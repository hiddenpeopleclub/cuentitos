...
pub enum PalabritasError
{
  ...
  CantReadFile{path: PathBuf, message: String},
  BucketSumIsNot1(ErrorInfo),
  BucketHasFrequenciesAndChances(ErrorInfo),
  BucketMissingProbability(ErrorInfo),
  UnexpectedRule{info: ErrorInfo,expected_rule: Rule, rule_found: Rule},
}
  ...
  fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
    match self {
        ...
        PalabritasError::BucketSumIsNot1(info) => {
            write!(f, "The sum of the probabilities in a bucket must be 100%.\n{}",info)
        }
        PalabritasError::BucketHasFrequenciesAndChances(info) => {
          write!(f, "Buckets can't have frequency notations and percentage notations at the same time.\n{}",info)
        }
        PalabritasError::UnexpectedRule{info,expected_rule, rule_found} => {
